...
RawTree trace acceptance is `sh scripts/accept-rawtree-trace.sh`. It runs local trace/redaction checks, then uses `RAWTREE_API_KEY` (or `--api-key-file /absolute/path` passed to the script) to insert a redacted development trace in the sandbox and query it back by run ID. Without a key it writes a blocked proof under `proof/slice-02` and exits 2. The proof contains no API key or media bytes. This is a representative replay of the source inspection proof, so its capture timing does not claim to measure the original inspection run.
