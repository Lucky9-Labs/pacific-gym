...
- `nimble-research` captions a local reference with Ollama, then uses Nimbleway Web Search Agents to find cited animation, art-direction, and Isaac Sim/physics resources. Only the text caption is sent to Nimble. It writes a research receipt and FLUX prompt draft; submitting a paid FLUX generation remains a separate explicit action.
